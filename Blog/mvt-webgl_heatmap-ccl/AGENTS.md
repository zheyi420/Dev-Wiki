...
| **数据规模** | 总览**主标题/文件名仍用「百万级」**；正文/副标题可写「当前方案验证规模约**三百万**条点数据」。Agent 内化主案例为**三百万级**；勿写「数十万条」 |
...
**勿写**：空间交互锁 — 非本系列目标。**勿写**比例尺档位与热区/点位等功能互斥（**§1.7**、**§2.3**）。**勿写** cnt 重复累计 / splat 虚高误述。
...
| 篇   | 关键验收                                         |
| --- | -------------------------------------------- |
| 00  | **开篇背景环境**（§1.6 全集表）+ 三百万主案例 + LOD 端到端图 + **单格网 vs 三格网**选型 + 导航 01–05 |
| 01  | **开篇背景环境**（PG/PostGIS）+ 双 MV + **三格网 MV 族** + 索引设计 + 刷新顺序（三格网 + 工单 SQL）+ §F LOD 动机 |
...
| **04**  | **开篇背景环境**（OL 10.6.1）+ **TileLayerBase 三段论** + **残留问题（非 cnt 虚高）** + 覆写对比表 + mermaid |
