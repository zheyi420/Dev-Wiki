...
| `02-bridge-filter-before-after.png` | 02 §3.4 | 筛选前后热力 + 同切片有无 `CQL_FILTER`（兼覆盖原 D1/D2，博文不重复插入） |
| `02-devtools-wfs-grid-in.png` | **02 §8.1**、05 §7 | 热区点击 WFS 仅格网标识 `IN` |
| `02-devtools-wfs-bbox.png` | 02 §8.2 | 视口 WFS `BBOX` + 场景 CQL |
| `01-devtools-fine-layer-url.png` | 02 §9（02-D3） | 细格网档位 GetTile `LAYER`（原 01 篇实拍） |
...
| 博文变更 | 插入 `./assets/00-overview-ui-chrome-networkpayload.png`；图注：*图：左下四维度筛选状态与 Network 中 MVT GetTile 载荷对照——`CQL_FILTER` 在查询层下推，瓦片 PBF properties 仍仅含格内工单数与格网标识。* |
...
| 标注版 | 否（源图已含左右对照与切片标注） |
| 博文变更 | 插入 `./assets/02-bridge-filter-before-after.png`；图注：*图：同一瓦片坐标下，未筛选时 GetTile 不带 `CQL_FILTER`（左），收窄专题后 URL 出现专题 ECQL 且屏上热力与「当前画面」合计随之变化（右）；筛选由服务端查询层生效并驱动瓦片 refresh 重绘，而非客户端合并要素缓存。* |
...
| 标注版 | 否（源图已含红字与箭头标注） |
| 博文变更 | 插入 `./assets/02-bridge-viewport-markers.png`；图注：*图：在同一筛选条件与当前视口下，将「热区色带阈值」「热力饱和标准」调至最低后，「热区统计」面板「当前画面」工单合计、左下「点位」查询的视口范围内工单总件数，与 WFS `BBOX` 探针返回的匹配总量一致（图中均为 23,007 件），说明视口热区掩膜汇总与 WFS 明细统计口径对齐。掩膜路径仅基于已加载 MVT 与 FBO 掩膜在客户端重算，无需每次全量拉取 WFS，响应更快，且在本验证条件下计数一致。* |
...
| 标注版 | 否 |
| 博文变更 | 插入 `./assets/01-devtools-fine-layer-url.png`；图注：*图：视口 `sourceZ` 较高时 GetTile 的 `LAYER` 指向细格网（约 5m）档位 MVT 图层。* |
...
| 画面应呈现 | 右栏「热区工单」列表；DevTools `CQL_FILTER` **仅** `grid_id IN (...)`，无四维度 ECQL 重复；`numberMatched`/`totalFeatures` 与列表条数一致（图中 35 件） |
| 文件名 | `02-devtools-wfs-grid-in.png` |
| 标注版 | 否（源图已含红框与箭头） |
| 博文变更 | 插入 `./assets/02-devtools-wfs-grid-in.png`；图注：*图：热区点击下钻时，尽管筛选 Dock 仍显示当前 MVT 筛选态，WFS 请求仅按关联格网标识 `IN (...)` 查询，不重复拼接事项专题/场景/年份/区县 ECQL。* |
...
| 标注版 | 否 |
| 博文变更 | 插入 `./assets/02-devtools-wfs-bbox.png`；图注：*图：视口工单 WFS 在 Dock 仅选业务场景时，`CQL_FILTER` 由 `BBOX` 与 `scene_type` 等属性条件组成，与热区点击的格网标识 `IN` 路径分离。* |

> **已取消条目**（无独立实拍图，且语义已由其它图覆盖）：**02-D1/D2**（`02-bridge-filter-before-after.png` 左右对照）、**02-D4**（粗档 LOD）、**02-D7**（查询层 vs 输出层 UI）。
...
| 预计新增图数 | 章节锚点 | 文件名 |
| --- | --- | --- |
| 8 | §3.3 | `00-overview-ui-chrome-networkpayload.png`（复用 00-U5） |
...
| | §9 | `01-devtools-fine-layer-url.png`（复用） |

**预计正文微调**：§8、§9 各引 DevTools 图注；§3.4 已含 D1/D2 语义，不另插 `wmts-cql/no-cql`。
...
| 00 总览 | 7 | 1 | 8 |
| 01 数据 | 1（复用） | 0 | 1 |
| 02 服务 | 3（含 1 张复用 00） | 3（含 1 张复用 01） | 8 |
| 03 前端上 | 3 | 2 | 5 |
| 04 前端中 | 2 | 2 | 4 |
| 05 前端下 | 6 | 1 | 7 |
| **合计** | **23** | **10** | **约 33 张**（含拼图源图与复用可减少实拍次数） |
