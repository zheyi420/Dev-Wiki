...
| **待执行** | 03–05 前端三篇 | 按 §8–§10 清单拍摄并插入博文 |
...
| `01-devtools-fine-layer-url.png` | 实拍（清单挂 02-D3） | 02 §9 |
...
**跨篇复用 3 组**：`map-heatmap`（00+01）、`networkpayload`（00+02）、`fine-layer-url`（02 引用；01 博文无 DevTools 插图）。
...
| `02-bridge-filter-before-after.png` | 02 §3.4、**03 §4**（待拍） | 筛选前后热力 + 同切片有无 `CQL_FILTER`（兼覆盖原 02-D1/D2） |
| `02-devtools-wfs-grid-in.png` | **02 §8.1**、**05 §7**（待拍） | 热区点击 WFS 仅关联格网标识 `IN` |
| `02-devtools-wfs-bbox.png` | 02 §8.2 | 视口 WFS `BBOX` + 场景 CQL |
| `01-devtools-fine-layer-url.png` | 02 §9（02-D3） | 细格网档位 GetTile `LAYER`（实拍文件挂 02；01 博文无 DevTools 插图） |
| `05-wfs-panel-stats.png` | **05 §7**（待拍） | 可与 drilldown 同源，侧重统计 Tab；00 已用 `drilldown` |

拍摄时可优先完成「复用源」，再按需裁切或重命名副本。**02 定稿**不引用 `03-filter-refresh-compare`、`04-devtools-lod-switch-layer` 等未纳入 02 博文的条目。
...
当前 [`./assets/`](./assets/) 中 **13 张 PNG 均属 00–02 定稿范围**；03–05 待拍文件尚未落盘。
...
**00 / 01 / 02 已完成**（配图落盘 + 博文已插入，见 §0）。**03–05 待执行**。图片路径统一为 `./assets/{文件名}`。
...
**00–02 已落盘资源**：[`./assets/`](./assets/) 共 **13 张**唯一 PNG，覆盖前三篇全部插图需求；跨篇复用见 §0.2。
...
| --- | --- | --- | --- |
| 00–02 已落实 | 11 | 4 | 13 张物理文件 / 15 处博文引用 |
| 03–05 待办 | 11 | 5 | 约 16 张（含复用可减少实拍） |
| **全系列** | **23** | **10** | **约 33 张**（含拼图源图与复用可减少实拍次数） |
