...
| **已定稿** | 03 前端 WebGL 渲染 | 5 张实拍 + 1 张 DevTools 复用，博文 6 处插图已插入（2026-09-02） |
...
当前 [`./assets/`](./assets/)：**00–03 共 18 张**唯一 PNG 已定稿；**04–05 待拍**。
...
> **03 定稿**：博文共 **6 处**插图引用（**5 张本篇实拍** + **1 张 DevTools 跨篇复用** = **6 张物理文件**）。已取消 **`03-devtools-tile-url-lod.png`** 单独拍摄。
...
| 插图数 | 章节锚点 | 文件名 | 博文状态 |
| --- | --- | --- | --- |
| 6 处引用 | §3 mermaid 后 | `03-pipeline-gradient.png` | 已插入 |
...
- §3 D2 复用 `01-devtools-fine-layer-url.png`；小结已提及缩放加载中间态直缝
...
| 03 前端上 | 4（含 `mvt-切片更新时效果.png`） | 1 实拍 + 1 复用 | 6 处引用 / 5 张本篇实拍 | **已落实** |

**00–03 已落盘资源**：[`./assets/`](./assets/) 共 **18 张**唯一 PNG（见 §0.2）；跨篇复用见 §2。
...
| --- | --- | --- | --- |
| 00–03 已落实 | 15 | 5 | 18 张物理文件 / 21 处博文引用 |
| 04–05 待办 | 8 | 3 | 约 11 张（含复用可减少实拍） |
| **全系列** | **23** | **8** | **约 18 张已落盘 + 约 11 张待拍** |
...
| 2026-09-02 | **03 博文插图插入**：6 处 `./assets/` 引用与图注；§4.1 含概念表 + 缩放 mermaid + U4 图；清单 §0 / §11.4 / §11.7 标为已插入 |
