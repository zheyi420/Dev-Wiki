...
如果只维护细格网这一档会怎样？在地图缩到很小比例时，一个瓦片会覆盖极大的地理范围，落在其中的细格网要素动辄成千上万。这些要素在屏幕上最终只糊成一片色块，却实打实地推高了瓦片体积、瓦片解码耗时与服务端缓存占用——付出的代价与看到的信息量完全不成比例。
...
值得注意的是**预聚合本身的压缩效果**：主验证数据集约三百万条点数据，聚合到细格网后约五十万行，量级上大约是 6:1；换到点数据更少、分布更集中的数据集，压缩比会有出入，但方向一致。这意味着即便只看细格网这一档，热力瓦片面对的要素规模也已经比原始点数据小了一个数量级；中、粗两档则是在此基础上进一步为低缩放档位减负。
...
格网标识不能用哈希函数（如对业务键做定长哈希后截断为 32 位整数）来生成——数十万级别的格网行规模下，32 位哈希空间的碰撞概率并不能忽略，一旦碰撞就会有两个不同的格子共享同一个数字标识，瓦片渲染与热区反查都会出错。
...
答案是**只指向细格网**，而且只维护这一份关联。这不是技术限制，而是明确的业务取舍：产品上只有在细格网呈现的缩放层级才提供热区点击下钻——粗、中格网服务的是「远看趋势」的场景，一个格子覆盖的范围太大，点开一份混杂的明细列表对用户没有意义。既然中、粗档位不需要反查，就没有必要为它们各维护一份关联，也就避免了三倍的构建耗时与存储开销（明细视图本身行数与源表同量级，首次构建是分钟级偏长的操作）。

对应到实现上，前端会用缩放门控挡住细格网档位之外的热区点击，保证发出去的那批格网标识一定属于细格网体系。这条链路的前端侧见 [05-前端-热区识别、计算与绘制](https://www.cnblogs.com/zheyi420/p/22182374)。
