...
内置 Heatmap 把「矢量全量 + splat + gradient」封在一层里；本方案的数据在 **MVT 瓦片**上，必须把 Heatmap 的 **GPU 数学**拆出来绑到 **WebGLVectorTile**，用子类补上官方未导出的 **postProcesses** 能力，再配合 **LOD URL 选层**，才能得到**可瓦片化、可筛选刷新、可后处理上色、可按缩放换档**的热力层。这一步同时是后两篇的前提：第 05 篇的热区识别要对「屏幕上真实看到的 alpha」做阈值分割，只有当 gradient 上色以 postProcess 形式跑在同一 WebGL 层内，CPU 侧才能拿到与视觉一致的累积 alpha——同一 alpha 也是 §3.1 所述视口快照热区分析的输入；第 04 篇则解决三档瓦片共存于同一瓦片源时「renderer 只画当前档」的问题。
...
### 3.1 视口快照：热区标注与边界的计算范围

若地图先放大至某一视口，再开启热力图功能模块，开启「显示热区工单数」与「显示热区边界」后，待热力加载与热区计算完成，再**缩小地图**，可能在缩放动画的某一帧看到如下画面：外围有新的热力晕染，但圆标数字与红色阶梯边界仍集中在屏幕中心一块区域——与当前大视口的外围热力不重合。
注意：此时对应的 mvt 依旧是同 LOD 图层、同 TILEMATRIX 级别。缩小程度未到需要请求新 TILEMATRIX 级别的地步。

![缩小瞬间：热力已扩至外围，圆标与边界仍停留在上一帧视口范围](./assets/掩膜计算边界.jpg)

*图：缩小地图的瞬间——热力图已随当前大视口重绘至外围（能够重绘至外围是因为之前视口下加载的 mvt 切片覆盖了外围，所以前端已有相应数据），热区工单数圆标与红色阶梯边界仍停留在**上一分析完成时**的中心区域。二者范围不一致，说明热力图跟**每帧 GPU 渲染**走（更外侧的地图上未显示热力渲染是因为那些区域的 mvt 切片还未请求回来），热区标注与边界跟**视口快照**走。*

这一帧并非常态。缩放结束后，热区管线会按新视口重新捕获 FBO 并重算标注与边界；下文借这一瞬间，说明热区标注与边界的**计算范围**为何天然限定在视口内。

#### 两条管线的分工

| 能力 | 渲染/计算来源 | 有效范围 | 典型更新时机 |
| --- | --- | --- | --- |
| **热力图（§3 splat + gradient）** | MVT 瓦片 + WebGL 每帧合成 | **当前视口**内有效瓦片 | 地图每帧 render；缩放时 `change:resolution` 额外标记图层 dirty |
| **热区工单数圆标 / 热区边界** | 某次 **FBO 整屏 readback** + CPU 连通域 + 视口内格网计数聚合 | **捕获时刻的整屏画面**（掩膜栅格边界即「计算边界」） | `moveend` / `tileloadend` 后，300ms 防抖，再 postrender readback |

缩小过程中，热力图已按**新的大视口**逐帧重绘；热区圆标与边界仍沿用**上一份缓存快照**，直到地图交互结束（`moveend`）并完成整条 FBO 管线。这是**高频 GPU 渲染**与**低频快照式分析**在触发时机上的差异，而非 MVT「加载过就永远画出来」。缩放结束后会重新对齐。

#### 掩膜计算边界 = 视口画面

§3 的 postProcess 产出的是屏幕上的**成品热力**。[第 05 篇](https://www.cnblogs.com/zheyi420/p/22182374)的热区识别**不是**对 MVT 格网做 GIS 邻接聚类，而是对**该成品 alpha** 做阈值分割与八连通域标注。因此「热区标注与边界只涉及视口内」有两层含义：

1. **FBO readback 天然是整屏**——捕获的是当时 WebGL canvas 上的热力画面；下采样得到的 alpha 掩膜栅格长宽与视口像素对应，**掩膜边界即视口矩形**。上图中外围虽有热力，但无圆标、无红色边界，因为该次快照完成时视口尚未扩至当前范围。
2. **格内工单数聚合再限视口**——连通域标签与 MVT 格网计数对齐时，只采集**当时视口 extent 与屏幕范围内**的格网要素，并与 renderer 的 active LOD 一致（LOD composite 见[第 04 篇](https://www.cnblogs.com/zheyi420/p/22699358)）。
...
热区边界经 `ImageCanvas` 绘制：捕获时将掩膜格四角固化为地理坐标，平移时会随地图重投影而跟手，但**数据范围仍来自该次快照**——缩小瞬间边界不会立刻扩至外围。坐标映射、掩膜下采样尺度、格网归属查表等实现细节见[第 05 篇](https://www.cnblogs.com/zheyi420/p/22182374)；本篇只建立「热力 GPU 管线 → 视口快照 → 标注/边界」的分工认知。
...
真正需要维护时序的只有两件事：筛选刷新后等待瓦片重新到达，再驱动依赖瓦片的下游计算（第 05 篇的「短暂归零」时序）；以及热力每帧重绘与热区视口快照在缩放过程中的短暂范围差异（§3.1）。
...
- 热力图每帧 GPU 渲染，热区圆标与边界来自视口 FBO 快照（§3.1）；缩小瞬间二者范围可能短暂不一致，缩放结束后重算对齐。
