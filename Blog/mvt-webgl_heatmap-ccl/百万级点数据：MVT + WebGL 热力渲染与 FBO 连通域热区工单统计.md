...
| **服务** | 热力瓦片 + 按需明细        | MVT 矢量瓦片 + GWC WMTS + `CQL_FILTER`；**查询层**保留全列筛选，**PBF 输出**仅「格内工单数」+ 格网标识 fid（详见 [02 篇](/Blog/mvt-webgl_heatmap-ccl/02-服务层-MVT瓦片与按需明细查询.md)） |
...
- 源业务表经 ETL 或定时任务写入 **格网聚合物化视图**（供热力 MVT）。**工单明细物化视图**在格网 MV 已存在且已刷新后构建：由源表关联格网 MV 取得**格网标识**并保留单条业务记录完整属性。日常数据更新时须 **先刷新格网 MV、再刷新工单明细 MV**（顺序原因见 [01 篇](/Blog/mvt-webgl_heatmap-ccl/01-数据层-双物化视图.md)）。
...
| **热区识别** | 格网中心点 GIS 聚类                        | splat 后 alpha 掩膜 + 8-连通 CCL | **方案 B**：热区是晕染后的视觉连通区域，阈值须划在 GPU 上色后的 alpha 上，标注才与屏幕所见一致（详见 [04 篇](/Blog/mvt-webgl_heatmap-ccl/04-前端-热区识别、计算与绘制.md)） |


**不选内置 Heatmap 的改造点**：数据在服务端已瓦片化，客户端须复用 Heatmap 的 splat 数学，但挂载到 **WebGL 矢量瓦片层**，并补上官方未暴露的 `postProcesses` 能力——这是 [03 篇](/Blog/mvt-webgl_heatmap-ccl/03-前端-热力图WebGL渲染管线.md) 的核心工作。
...
| 篇      | 链接                                                                                                       | 本篇解决什么                                           |
| ------ | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| **01** | [/Blog/mvt-webgl_heatmap-ccl/01-数据层-双物化视图.md](/Blog/mvt-webgl_heatmap-ccl/01-数据层-双物化视图.md)               | 为何两个物化视图；格网标识与索引设计；源数据更新后的刷新顺序                   |
| **02** | [/Blog/mvt-webgl_heatmap-ccl/02-服务层-MVT瓦片与按需明细查询.md](/Blog/mvt-webgl_heatmap-ccl/02-服务层-MVT瓦片与按需明细查询.md) | 查询层 vs 输出层；MVT 属性裁剪；MVT 与 WFS 分工                 |
| **03** | [/Blog/mvt-webgl_heatmap-ccl/03-前端-热力图WebGL渲染管线.md](/Blog/mvt-webgl_heatmap-ccl/03-前端-热力图WebGL渲染管线.md)   | 为何不用内置 Heatmap；OL 覆写与 splat 管线；瓦片 GPU 融合         |
| **04** | [/Blog/mvt-webgl_heatmap-ccl/04-前端-热区识别、计算与绘制.md](/Blog/mvt-webgl_heatmap-ccl/04-前端-热区识别、计算与绘制.md)       | FBO readback + CCL；ImageCanvas 热区边界；热区点击与双 MV 回扣 |
