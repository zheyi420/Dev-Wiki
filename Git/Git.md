...
- 这种方式会将目标分支和功能分支的历史合并，生成一个新的“合并提交”（merge commit），该提交有两个父提交，分别指向两个分支的最新提交。
- 这样做可以完整保留两个分支的所有提交历史，但会在提交历史中产生额外的合并节点，可能导致提交记录较为杂乱。
- 适合需要保留完整分支历史和合并过程的场景。

### 2. Squash and merge（压缩合并）
- 这种方式会将功能分支上的所有提交“压缩”为一个单一的提交，然后将这个合并后的单一提交合并到目标分支。
- 合并后，目标分支的提交历史线性且简洁，只显示一个代表整个PR的提交，丢弃了功能分支上的中间提交细节。
- 适合希望保持干净、简洁的提交历史，或者功能分支上的提交是一些零散的工作进展，不需要保留每个小提交的情况。

### 3. Rebase and merge（变基合并）
- 这种方式会将功能分支上的每个提交逐个“变基”到目标分支的最新提交之后，也就是将功能分支的提交“重新应用”在目标分支上，保持提交的顺序和细节，但不会产生额外的合并提交。
- 结果是目标分支的提交历史线性，没有合并节点，但保留了功能分支上的所有独立提交。
- 适合需要保留每个提交的详细信息，同时又想保持提交历史线性的场景。
...
|--------------------|--------------------------------|---------------------------------|--------------------------------|
| 提交数             | 保留所有原始提交 + 新合并提交   | 只保留一个压缩后的提交           | 保留所有原始提交，无合并提交    |
| 提交历史形态       | 非线性，有合并节点              | 线性，只有一个合并提交           | 线性，所有提交依次排列          |
| 提交信息           | 保留所有提交信息                | 由用户自定义单个提交信息         | 保留所有原始提交信息            |
| 适用场景           | 需要完整保留分支合并历史        | 需要简洁、干净的提交历史         | 需要保留详细提交且保持线性历史  |
| 是否产生合并提交   | 是                             | 否                              | 否                             |

---

### 说明
- **Create a merge commit**是Git默认的合并方式，使用`--no-ff`参数，保留完整分支历史。
- **Squash and merge**将功能分支的多个提交压缩成一个提交，保持历史简洁，适合不需要保留每个小提交的场景。
- **Rebase and merge**通过变基操作将功能分支的提交线性地附加到目标分支，保持提交的细节且无合并提交，适合需要保持线性历史同时保留每个提交的情况。

这三种方式各有优劣，具体选择取决于团队对提交历史的管理需求和偏好。
